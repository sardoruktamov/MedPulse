...
public class PatientProfileController {
    ...
    public ResponseEntity<List<PatientProfileEntity>> getMyProfiles() {
        return ResponseEntity.ok(patientProfileService.getMyFamilyProfiles());
    }

    // 2. Profilni to'ldirish (Update)
    @PutMapping("/{id}")
    public ResponseEntity<PatientProfileEntity> updateProfile(
    ...
    @PostMapping("/family")
    public ResponseEntity<PatientProfileEntity> createMember(@Valid @RequestBody PatientCreateDTO dto) {
